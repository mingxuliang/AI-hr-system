...
def generate_jd_route(request: JDGenerateRequest):
...
def generate_jd_stream_route(request: JDGenerateRequest):
...
def chat_jd_stream_route(request: JDChatRequest):
    return StreamingResponse(
        chat_jd_stream(
...
def get_position_stats_route(position_id: UUID, db: Session = Depends(get_db)):
    position = get_position(db, position_id)
    if not position:
        raise HTTPException(status_code=404, detail="Position not found")
...
def get_position_question_banks_route(position_id: UUID, db: Session = Depends(get_db)):
    position = get_position(db, position_id)
    if not position:
        raise HTTPException(status_code=404, detail="Position not found")
